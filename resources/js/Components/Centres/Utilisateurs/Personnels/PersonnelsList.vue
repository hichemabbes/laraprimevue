            ...
            >
                <template #empty>
                    <div class="text-center py-4">
                        ...
                        <p class="text-600">Aucun personnel trouvé pour le centre ID {{ centreId || 'inconnu' }}.</p>
                    </div>
                </template>
    ...
    methods: {
        ...
        async fetchPersonnels(event = {}) {
            ...
            try {
                ...
                const response = await axios.get('/api/personnels-centres', {
                ...
                this.handleApiError(error, 'Erreur lors du chargement des personnels.');
                this.personnels = [];
                this.totalRecords = 0;
            } finally {
                this.loading = false;
        ...
        async fetchAnneesFormations() {
            ...
            try {
                const response = await axios.get('/api/personnels-centres/annees-formations', {
                    headers: this.getHeaders(),
                });
        ...
        async openFonctionForm(personnel) {
            if (!personnel?.id) {
                this.showError('Aucun personnel sélectionné ou ID manquant.');
                return;
            }
        ...
        async openRegimeForm(personnel) {
            ...
            this.loading = true;
            try {
                const response = await axios.get(`/api/personnels-centres/${personnel.id}/regimes`, {
                    headers: this.getHeaders(),
                });
        ...
        async deletePersonnel() {
            ...
            this.loading = true;
            try {
                const response = await axios.delete(`/api/personnels-centres/${this.personnelToDelete.id}`, {
                    headers: this.getHeaders(),
                    data: { password: this.deletePassword.trim() },
                });
                this.personnels = this.personnels.filter(p => p.id !== this.personnelToDelete.id);
                this.totalRecords = Math.max(0, this.totalRecords - 1);
                this.showSuccess(response.data.message || 'Personnel supprimé avec succès.');
                this.deleteFormVisible = false;
                this.personnelToDelete = null;
                this.deletePassword = '';
                this.passwordError = '';
        ...
        async exportPersonnels() {
            try {
                const response = await axios.get('/api/personnels-centres/export', {
                    headers: this.getHeaders(),
                    responseType: 'blob',
                });
                ...
                link.href = url;
                link.setAttribute('download', 'personnels.xlsx');
        ...
        async onImport(event) {
            ...
            try {
                const response = await axios.post('/api/personnels-centres/import', formData, {
