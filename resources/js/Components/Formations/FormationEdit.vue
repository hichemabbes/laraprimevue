    ...
    >
        ...
        <template #header>
            <div class="flex align-items-center gap-2">
                ...
                <span class="font-bold text-2xl">Modifier la Formation</span>
            </div>
        </template>
...
export default {
    ...
    methods: {
        ...
        fillFormFromFormation() {
            if (!this.formationData) return;
            this.form = {
                session_id:
                    this.formationData.session_id ||
                    this.formationData.session_formation_id ||
                    this.formationData.session?.id ||
                    null,
                specialite_id:
                    this.formationData.specialite_id ||
                    this.formationData.specialite?.id ||
                    null,
                theme_id:
                    this.formationData.theme_id || this.formationData.theme?.id || null,
                code: this.formationData.code || '',
                nom_fr: this.formationData.nom_fr || '',
                nom_ar: this.formationData.nom_ar || '',
                cible_fr: this.formationData.cible_fr || '',
                cible_ar: this.formationData.cible_ar || '',
                statut: this.formationData.statut || 'Actif',
                observation_fr: this.formationData.observation_fr || '',
                observation_ar: this.formationData.observation_ar || '',
            };
        },
        async submitForm() {
            if (!this.formationId) {
            ...
            try {
                const res = await axios.put(
                    `/api/formations/${this.formationId}`,
                    this.form
                );
