    ...
    >
        ...
        <template #header>
            <div class="flex align-items-center gap-2">
                <i class="pi pi-calendar-edit text-primary text-2xl"></i>
                <span class="font-bold text-2xl">Modifier la Session</span>
            </div>
        </template>
...
export default {
    ...
    methods: {
        fillFormFromSession() {
            ...
            this.form = {
                annee_formation_id:
                    this.sessionData.annee_formation_id ||
                    this.sessionData.annee_formation?.id ||
                    null,
                code: this.sessionData.code || '',
                intitule_fr: this.sessionData.intitule_fr || '',
                intitule_ar: this.sessionData.intitule_ar || '',
                date_debut: this.sessionData.date_debut
                    ? new Date(this.sessionData.date_debut)
                    : null,
                date_fin: this.sessionData.date_fin
                    ? new Date(this.sessionData.date_fin)
                    : null,
                capacite_min: this.sessionData.capacite_min ?? null,
                capacite_max: this.sessionData.capacite_max ?? null,
                statut: this.sessionData.statut || 'Planifiee',
                observation_fr: this.sessionData.observation_fr || '',
                observation_ar: this.sessionData.observation_ar || '',
            };
        },
        async loadAnnees() {
            this.loading = true;
        ...
        async submitForm() {
            ...
            try {
                ...
                const res = await axios.put(`/api/sessions/${this.sessionId}`, payload);

                this.$emit('update', res.data);
                this.toast.add({
                    severity: 'success',
                    summary: 'Succès',
                    detail: 'Session mise à jour avec succès.',
                    life: 3000,
                });
