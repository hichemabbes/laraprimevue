        ...
        <div v-else class="surface-card p-4 shadow-2 border-round">
            <form @submit.prevent="submitForm">
                <div class="flex flex-column gap-4">
                    ...
                    <div class="field">
                        <label
                            for="type_repos_formation_fr"
                            class="block font-medium mb-2"
                        >
                            Type de Repos <span class="text-red-500">*</span>
                        </label>
                        <Dropdown
                            id="type_repos_formation_fr"
                            v-model="form.type_repos_formation_fr"
                            :options="typesRepos"
                            optionLabel="nom_fr"
                            optionValue="nom_fr"
                            ...
                            class="w-full"
                            :class="{
                                'p-invalid': errors.type_repos_formation_fr,
                            }"
                        ...
                        />
                        <small
                            v-if="errors.type_repos_formation_fr"
                            class="p-error"
                        >
                            {{ errors.type_repos_formation_fr[0] }}
                        </small>
                        <small
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const isFormValid = computed(
            () =>
                !isSaving.value &&
                form.value.type_repos_formation_fr &&
        ...
        watch(
            ...
            async (newVal) => {
                if (newVal) {
                    ...
                    if (props.annee) {
                        form.value.annee_formation_id = props.annee.id;
                    }
        ...
        watch(
            ...
            (newVal) => {
                if (newVal) {
                    form.value.annee_formation_id = newVal.id;
                }
            }
        );
            ...
            try {
                ...
                if (types.length === 0) {
                    toast.add({
                        ...
                        detail: 'Aucun type de repos disponible. Veuillez vérifier la configuration des listes.',
                        life: 5000,
                    });
                }
            } catch (error) {
        ...
        function resetForm() {
            form.value = {
                annee_formation_id: props.annee?.id || null,
                type_repos_formation_fr: null,
                date_debut: null,
                date_fin: null,
                description: '',
    ...
    methods: {
        ...
        validateTypeFr() {
            if (!this.form.type_repos_formation_fr) {
                this.errors.type_repos_formation_fr = [
                    'Le type de repos est obligatoire',
                ];
            } else if (
                !this.typesRepos.some(
                    (type) => type.nom_fr === this.form.type_repos_formation_fr
                )
            ) {
                this.errors.type_repos_formation_fr = [
            ...
            try {
                ...
                if (this.isEditing) {
                    response = await axios.put(
                        `/api/periodes-repos/${this.periodeToEdit.id}`,
                        payload
                    );
                    this.$emit('update', response.data);
                } else {
                    response = await axios.post('/api/periodes-repos', payload);
