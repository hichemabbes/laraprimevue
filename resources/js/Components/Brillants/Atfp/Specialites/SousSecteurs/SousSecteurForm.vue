<template>
    ...
    >
        ...
        <div class="flex flex-column gap-3">
            <!-- Row 1: Standardisation (AR) and Secteur (AR) -->
            <div class="grid">
                <div class="col-12 md:col-6">
                    <div class="field mb-0">
                        ...
                        >
                            Type de secteur <span class="text-red-500">*</span>
                        </label>
                        <Dropdown
                ...
                <div class="col-12 md:col-6">
                    <div class="field mb-0">
                        <label for="secteur_id" class="block font-medium mb-2">
                            Secteur (AR) <span class="text-red-500">*</span>
                        </label>
                        <Dropdown
                            id="secteur_id"
                        ...
                        />
                        <small v-if="errors.secteur_id" class="p-error">{{
                            errors.secteur_id
                        }}</small>
                    </div>
                </div>
            ...
            <!-- Row 4: Date Création and Date Annulation -->
            <div class="grid">
                <div class="col-12 md:col-6">
                    <div class="field mb-0">
                        <label
...
export default {
    ...
    data() {
        return {
            ...
            secteurs: [],
            standardisations: ['مقيس', 'غير مقيس'],
    ...
    computed: {
        ...
        filteredSecteurs() {
            console.log(
                'Computing filteredSecteurs, standardisation_ar:',
                this.newElement.standardisation_ar
            );
            return this.newElement.standardisation_ar
                ? this.secteurs.filter(
                      (s) =>
                          s.standardisation_ar ===
                          this.newElement.standardisation_ar
                  )
                : this.secteurs;
        },
    ...
    watch: {
        initialData: {
            ...
            async handler(newVal) {
                console.log('initialData received:', newVal);
                if (!this.secteursLoaded) {
                    await this.fetchSecteurs();
                }
                if (newVal) {
                    const standardisation_ar =
                        newVal.secteur?.standardisation_ar || null;
                    console.log(
                        'Setting standardisation_ar:',
                        standardisation_ar
                    );
                    console.log('Setting secteur_id:', newVal.secteur_id);
                    this.newElement = {
                        id: newVal.id || null,
                        standardisation_ar: standardisation_ar,
                    ...
                    this.$nextTick(() => {
                        console.log(
                            'filteredSecteurs after update:',
                            this.filteredSecteurs
                        );
                        console.log(
                            'newElement after update:',
                            this.newElement
                        );
                        this.validateStandardisationAr(true);
                        this.validateSecteurId(true);
                        this.validateCode();
                        this.validateNomFr();
                        this.validateNomAr();
                        this.validateStatut();
        ...
        'newElement.standardisation_ar'(newVal, oldVal) {
            console.log(
                'standardisation_ar changed from',
                oldVal,
                'to',
                newVal
            );
            if (this.isEditing && newVal !== oldVal && oldVal !== null) {
                this.newElement.secteur_id = null;
            }
        ...
        'newElement.secteur_id'(newVal) {
            console.log('secteur_id changed to:', newVal);
    ...
    methods: {
        async fetchSecteurs() {
            try {
                ...
                this.secteurs = Array.isArray(response.data)
                    ? response.data
                    : [];
                this.secteursLoaded = true;
                console.log('Secteurs loaded:', this.secteurs);
            } catch (error) {
                this.$emit('error', 'Impossible de charger les secteurs.');
        ...
        validateCode: debounce(async function () {
            this.errors.code = '';
            ...
            if (
                ...
                this.errors.code =
                    'Sélectionnez un type de secteur et un secteur avant de vérifier le code.';
                return;
            }
            if (this.isEditing) {
                return;
            ...
            try {
                const response = await axios.post(
                    '/api/sous-secteurs/check-code',
                    {
                        code: this.newElement.code,
                        secteur_id: this.newElement.secteur_id,
                ...
                this.errors.code = 'Erreur lors de la vérification du code.';
        ...
        validateNomFr() {
            this.errors.nom_fr =
                this.newElement.nom_fr && this.newElement.nom_fr.length > 100
                    ? 'Le nom en français ne doit pas dépasser 100 caractères.'
                    : '';
        },
        validateNomAr() {
            this.errors.nom_ar = !this.newElement.nom_ar
                ? 'Le nom en arabe est requis.'
                : this.newElement.nom_ar.length > 100
                  ? 'Le nom en arabe ne doit pas dépasser 100 caractères.'
                  : '';
        },
        validateStandardisationAr(silent = false) {
            if (!this.newElement.standardisation_ar) {
                this.errors.standardisation_ar = silent
                    ? ''
                    : 'Le type de secteur est requis.';
            } else if (
                !this.standardisations.includes(
                    this.newElement.standardisation_ar
                )
            ) {
                this.errors.standardisation_ar = silent
                    ? ''
                    : 'Le type doit être "مقيس" ou "غير مقيس".';
            } else {
                this.errors.standardisation_ar = '';
            }
        },
        validateSecteurId(silent = false) {
            if (!this.newElement.secteur_id) {
                this.errors.secteur_id = silent
                    ? ''
                    : 'Le secteur parent est requis.';
            } else if (
                !this.secteurs.find((s) => s.id === this.newElement.secteur_id)
            ) {
                this.errors.secteur_id = silent ? '' : 'Secteur invalide.';
            } else if (
                this.newElement.standardisation_ar &&
                !this.filteredSecteurs.find(
                    (s) => s.id === this.newElement.secteur_id
                )
            ) {
                this.errors.secteur_id = silent
                    ? ''
                    : 'Le secteur sélectionné ne correspond pas au type de secteur.';
            } else {
        ...
        validateStatut() {
            this.errors.statut =
                ...
                !this.statuts.includes(this.newElement.statut)
