            ...
            >
                <template #empty>
                    <div class="text-center py-4">
                        ...
                        <p class="text-600">Aucun secteur trouvé.</p>
                    </div>
                </template>
                ...
                >
                    <template #body="{ data }">
                        <Tag
                            :value="data.standardisation_fr"
                            :icon="
                                getStandardisationIcon(data.standardisation_fr)
                            "
                            class="standardisation-fr"
                            :data-standardisation="data.standardisation_fr"
                        />
                    </template>
                    <template #filter="{ filterModel, filterCallback }">
                        <Dropdown
                ...
                <div class="confirmation-content flex flex-column gap-3 p-3">
                    <div class="flex align-items-center gap-3">
                        ...
                        <span v-else-if="selected?.length > 0">
                            ...
                            <b>{{ selected.length }}</b> secteurs sélectionnés ?
                        </span>
                    </div>
                    <div class="field">
                    ...
                    >
                        <template #body="{ data }">
                            <Tag
                                :value="data.standardisation_fr"
                                :icon="
                                    getStandardisationIcon(
                                        data.standardisation_fr
                                    )
                                "
                                class="standardisation-fr"
                                :data-standardisation="data.standardisation_fr"
...
import Form from '@/Components/Atfp/Specialites/Secteurs/SecteurForm.vue';
import ImportError from '@/Components/Atfp/ImportError/SecteursImportError.vue';
...
export default {
    ...
    data() {
        return {
            ...
            standardisations: ['Standardisé', 'Non Standardisé'],
            actions: [
                {
                    label: 'Modifier',
                    icon: 'pi pi-pencil',
    ...
    methods: {
        ...
        async fetchSecteurs() {
            ...
            try {
                ...
                this.toast.add({
                    severity: 'success',
                    summary: 'Succès',
                    detail: 'Liste des secteurs chargée.',
                    life: 3000,
                });
            } catch (error) {
        ...
        async fetchTrashedSecteurs() {
            this.trashedLoading = true;
            try {
                console.log('Envoi de la requête à /secteurs/trashed');
                const response = await axios.get('api/secteurs/trashed');
                console.log('Réponse de /secteurs/trashed:', response.data);
                this.trashedSecteurs = Array.isArray(response.data)
                    ? response.data
                    : [];
                this.trashedSecteurs.forEach((sector) => {
                    console.log('Secteur ID:', sector.id);
                });
                this.toast.add({
                    severity: 'success',
                    summary: 'Succès',
                    detail: 'Liste des secteurs supprimés chargée.',
                    life: 3000,
                });
            } catch (error) {
                console.error(
                    'Erreur lors du chargement des secteurs supprimés:',
                    error.response?.data || error
                );
                let errorMessage =
                    'Impossible de charger les secteurs supprimés.';
        ...
        async restoreSecteur(secteur) {
            if (!secteur || !Number.isInteger(secteur.id)) {
                console.error('ID de secteur invalide:', secteur);
                this.toast.add({
                    severity: 'error',
                    summary: 'Erreur',
                    detail: 'ID de secteur invalide.',
                    life: 3000,
                });
                return;
            }
            console.log(
                'Tentative de restauration du secteur avec ID:',
                secteur.id
            );
            try {
                await axios.post(`api/secteurs/restore/${secteur.id}`);
                this.trashedSecteurs = this.trashedSecteurs.filter(
                    (s) => s.id !== secteur.id
                );
                ...
                this.toast.add({
                    severity: 'success',
                    summary: 'Succès',
                    detail: 'Secteur restauré.',
                    life: 3000,
                });
            } catch (error) {
                console.error(
                    'Erreur lors de la restauration du secteur:',
                    error.response?.data || error
                );
                this.toast.add({
                    severity: 'error',
                    ...
                    detail:
                        ...
                        'Impossible de restaurer le secteur.',
                    life: 3000,
                });
            }
        },
        openTrashedDialog() {
            ...
            this.fetchTrashedSecteurs();
        },
        async handleSave(newElement) {
            try {
                await axios.post('/secteurs', newElement);
                await this.fetchSecteurs();
                this.formVisible = false;
                this.toast.add({
                    severity: 'success',
        ...
        async handleUpdate(updated) {
            try {
                await axios.put(`api/secteurs/${updated.id}`, updated);
                await this.fetchSecteurs();
                this.formVisible = false;
                this.toast.add({
                    severity: 'success',
                    summary: 'Succès',
                    detail: 'Secteur modifié.',
                    life: 3000,
                });
            } catch (error) {
                console.error(
        ...
        async exportXls() {
            try {
                ...
                worksheet.columns = [
                    ...
                    {
                        header: 'Standardisation (FR)',
                        key: 'standardisation_fr',
                        width: 20,
                    },
                ...
                this.table.forEach((s) => {
                    worksheet.addRow({
                        ...
                        nom_fr: s.nom_fr,
                        nom_ar: s.nom_ar,
                        standardisation_fr: s.standardisation_fr,
                        statut: s.statut,
        ...
        getStandardisationIcon(conventionalisation) {
            const standardisationIcons = {
                Standardisé: 'pi pi-check',
                'Non Standardisé': 'pi pi-times',
            };
            return standardisationIcons[conventionalisation] || 'pi pi-info';
        },
        formatDate(date) {
            if (!date) return null;
...
.standardisation-fr[data-standardisation='Standardisé'] {
    background-color: #0681fc;
    color: #ffffff;
}
.standardisation-fr[data-standardisation='Non Standardisé'] {
