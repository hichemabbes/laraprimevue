                ...
                <div class="flex align-items-center gap-3">
                    ...
                    <div>
                        <h2 class="m-0 text-900 font-bold">
                            Gestion des Documents des Modules
                        </h2>
                        <div class="flex align-items-center gap-2 mt-1">
                            ...
                            />
                            <span class="text-700">{{
                                    module?.titre_module || 'Module non défini'
                                }}</span>
                        </div>
                    </div>
                </div>
        ...
        >
            <template #header>
                <div class="flex align-items-center gap-2">
                    ...
                    <span class="font-semibold">Détails du module</span>
                    <Badge
                        :value="documentCount"
                        severity="info"
                        class="ml-2"
                ...
                <div class="col-12 md:col-3">
                    <div class="detail-item">
                        ...
                        <p class="detail-value font-bold text-900">
                            {{ module?.code || 'Non défini' }}
                        </p>
                    </div>
                </div>
                <div class="col-12 md:col-3">
                    <div class="detail-item">
                        <label class="text-600 text-sm font-medium">Titre</label>
                        <p class="detail-value text-900">
                            {{ module?.titre_module || 'Non défini' }}
                        </p>
                    </div>
                </div>
                <div class="col-12 md:col-3">
                    <div class="detail-item">
                        <label class="text-600 text-sm font-medium">Type</label>
                        <Tag
                            :value="module?.type_module || 'Non défini'"
                            severity="info"
                        />
                    </div>
                </div>
                ...
                <div class="flex align-items-center justify-content-between w-full">
                    <div class="flex align-items-center gap-2">
                        ...
                        <span class="font-bold text-lg">{{
                                editMode ? 'Modifier un document' : 'Ajouter un document'
                            }}</span>
                    </div>
                    <Button
                        icon="pi pi-times"
                ...
                <div class="flex flex-wrap gap-3 w-full">
                    <div class="field flex-1">
                        ...
                        <InputText
                            ...
                            placeholder="Ex: Document du module"
                            class="w-full"
    ...
    setup(props, { emit }) {
        ...
        const api = {
            ...
            async fetchDocumentTypes() {
                const response = await axios.get('/api/document-module');
                return Array.isArray(response.data.data) ? response.data.data : [];
            },
            async fetchDocuments(moduleId) {
                const response = await axios.get(`/api/documents-modules?module_id=${moduleId}`);
                return Array.isArray(response.data.data) ? response.data.data : [];
            },
            async saveDocument(formData, isEdit, documentId) {
                return axios({
                    method: 'post',
                    url: isEdit ? `/api/documents-modules/${documentId}` : '/api/documents-modules',
                    data: formData,
                    headers: { 'Content-Type': 'multipart/form-data' },
                });
            },
            async deleteDocument(documentId) {
                return axios.delete(`/api/documents-modules/${documentId}`);
            },
            async downloadDocument(documentId) {
                return axios.get(`/api/documents-modules/${documentId}/download`, {
                    responseType: 'blob',
                });
            },
        };
        ...
        const fetchDocumentTypes = async () => {
            try {
                ...
                handleApiError(error, 'Impossible de charger les types de documents.');
            }
        };

        const fetchDocuments = async () => {
            if (!props.module?.id) {
                console.warn('Aucun module_id fourni:', props.module);
                toast.add({
                    severity: 'warn',
                    summary: 'Avertissement',
                    detail: 'Aucun module sélectionné pour charger les documents.',
                    life: 3000,
                });
                documents.value = [];
                return;
            ...
            loading.value = true;
            try {
                documents.value = await api.fetchDocuments(props.module.id);
        ...
        const saveDocument = async () => {
            ...
            if (!props.module?.id) {
                console.error('Erreur: module_id est indéfini', { module: props.module });
                toast.add({
                    severity: 'error',
                    summary: 'Erreur',
                    detail: 'ID de module manquant. Veuillez sélectionner un module.',
                    life: 3000,
                });
                return;
            }
            ...
            formData.append('module_id', props.module.id.toString());
            ...
            try {
                ...
                emit(editMode.value ? 'update' : 'save', response.data);
            } catch (error) {
        ...
        const editDocument = (doc) => {
            if (!doc?.id || !props.module?.id) {
                toast.add({
                    severity: 'error',
                    summary: 'Erreur',
                    detail: 'Document ou module invalide.',
                    life: 3000,
                });
                return;
            }
        ...
        watch(
            ...
            (newId) => {
                if (newId) {
                    documentForm.value.module_id = newId;
                    fetchDocuments();
                } else {
                    documents.value = [];
                    documentForm.value.module_id = null;
                }
            },
            { immediate: true }
        );
        ...
        onMounted(async () => {
            initFilters();
            console.log('Composant monté, module:', props.module);
            try {
                await api.initializeCsrf();
                await fetchDocumentTypes();
                if (props.module?.id) {
                    documentForm.value.module_id = props.module.id;
                    await fetchDocuments();
                }
            } catch (error) {
