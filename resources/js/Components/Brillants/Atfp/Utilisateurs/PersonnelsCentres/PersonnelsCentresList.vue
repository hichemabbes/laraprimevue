        ...
        <div class="surface-card p-4 pt-2 border-round shadow-2 border-1 surface-border">
            ...
            <DataTable
                ...
                :globalFilterFields="[
                    ...
                    'qualite_fr', 'etablissement_origine_fr', 'usersCentre.centre.nom_fr',
                    'usersCentre.centre_statut_fr', 'usersCentre.personnelCentre.etat_personnel_fr',
                    'usersCentre.personnelCentre.for_cons_fr'
                ]"
                :loading="loading"
                scrollable
                scrollHeight="750px"
            ...
            >
                ...
                <!-- Colonne Centre -->
                <Column
                    field="usersCentre.centre.nom_fr"
                    header="Centre"
                    sortable
                    style="min-width: 15rem"
                >
                    <template #body="{ data }">
                        <span v-if="data.usersCentre && data.usersCentre.centre">
                            {{ data.usersCentre.centre.nom_fr }}
                        </span>
                        <span v-else>-</span>
                    </template>
                </Column>
                ...
                >
                    <template #body="{ data }">
                        <span>{{ data.qualite_fr || '-' }}</span>
                    </template>
                </Column>
                <!-- Colonne Établissement d'origine -->
                <Column
                ...
                >
                    <template #body="{ data }">
                        <span v-if="data.qualite_fr === 'Personnel (Externe)'">
                            {{ data.etablissement_origine_fr || '-' }}
                        </span>
                        <span v-else>-</span>
                    </template>
                </Column>
                <!-- Colonne État du personnel -->
                <Column
                    field="usersCentre.personnelCentre.etat_personnel_fr"
                    header="État Personnel"
                    sortable
                    style="min-width: 15rem"
                >
                    <template #body="{ data }">
                        <span v-if="data.usersCentre && data.usersCentre.personnelCentre">
                            {{ data.usersCentre.personnelCentre.etat_personnel_fr || '-' }}
                        </span>
                        <span v-else>-</span>
                    </template>
                </Column>
                <!-- Colonne Formateur/Conseiller -->
                <Column
                    field="usersCentre.personnelCentre.for_cons_fr"
                    header="Formateur/Conseiller"
                    sortable
                    style="min-width: 15rem"
                >
                    <template #body="{ data }">
                        <span v-if="data.usersCentre && data.usersCentre.personnelCentre">
                            {{ data.usersCentre.personnelCentre.for_cons_fr || '-' }}
                        </span>
                        <span v-else>-</span>
                    </template>
                </Column>
...
import PersonnelCentreAdd from './PersonnelCentreAdd.vue';
import PersonnelCentreEdit from './PersonnelCentreEdit.vue';
import ProfilPersonnelCentre from './ProfilPersonnelcentre.vue';
import ImportErrors from '@/Components/Atfp/Utilisateurs/PersonnelsCentres/PersonnelsCentreImportError.vue';
export default {
    components: {
        Button,
        InputText,
    ...
    mounted() {
        ...
        if (userData) {
            try {
                ...
                this.isCentreRole = user.isCentreRole === 1;
    ...
    methods: {
        // Nouvelle méthode pour mettre à jour les erreurs et les sauvegarder
        ...
        async fetchUsers() {
            this.loading = true;
            try {
                const { data } = await axios.get('/api/personnels_centres-with-options', {
                    headers: {
                ...
                this.users = data.data.personnels.data.map(user => ({
                    ...
                    usersCentre: user.usersCentre || null
                }));
        ...
        openEditForm(user) {
            ...
            try {
                this.userToEdit = { ...user, roles: Array.isArray(user.roles) ? user.roles.map(role => role.id) : [] };
        ...
        onUserUpdated(user) {
            try {
                ...
                if (index !== -1) {
                    this.users.splice(index, 1, { ...user, roles: Array.isArray(user.roles) ? user.roles : [] });
                    this.users = this.sortUsersByPinned(this.users);
                }
        ...
        async deleteUser() {
            ...
            try {
                ...
                const response = await axios.delete(`/api/personnels_centres/${this.userToDelete.id}`, {
        ...
        async triggerFileImport(event) {
            ...
            try {
                const response = await axios.post('/api/personnels_centres/importxls', formData, {
                    headers: {
        ...
        async downloadTemplate() {
            try {
                const response = await axios.get('/api/personnels_centres/download-template', {
                    responseType: 'blob',
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('token')}`
                    }
        ...
        async exportXLS() {
            try {
                const response = await axios.get('/api/personnels_centres/exportxls', {
                    responseType: 'blob',
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('token')}`
                    }
