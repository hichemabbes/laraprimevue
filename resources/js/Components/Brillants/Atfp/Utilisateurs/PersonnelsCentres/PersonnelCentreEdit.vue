    ...
    >
        ...
        <template #header>
            <div class="flex align-items-center gap-2">
                ...
                <span class="font-bold text-2xl">Modifier un Utilisateur</span>
            </div>
        </template>
        ...
        <div v-else class="surface-card p-4 shadow-2 border-round">
            <form @submit.prevent="submitForm">
                <TabView ref="tabview" v-model:activeIndex="activeTabIndex">
                    <!-- IU -->
                    <TabPanel header="Information Personnel">
        ...
        const submitForm = async () => {
            ...
            try {
                ...
                if (form.value.statut === 'Actif') {
                    form.value.cause_inactivite_fr = '';
                    form.value.cause_inactivite_ar = '';
                    form.value.date_fin_service = null; // Ajout de la réinitialisation
                }
                const payload = {
                ...
                const response = await axios.put(`/api/personnels_atfp/${form.value.id}`, payload, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('token')}`,
                    },
                });
            ...
            async (newVisible) => {
                if (newVisible) {
                    ...
                    if (props.userId || props.userToEdit?.id) {
                        ...
                        form.value = { ...form.value, ...props.userToEdit };
                        await validateForm();
                    }
                } else {
                    resetForm();
